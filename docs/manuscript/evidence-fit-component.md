...
A subsequent AI review identified a low-severity diagnostic defect: filesystem failures could lose their operational error category. The corrected development source preserves that category, while the original evaluation remains bound to its original source. A sealed source archive and separate replay wrapper reproduce the original run; a distinct compatibility check compares current-API reports with the retained reports. The [post-run clarification](../../evaluation/evidence-fit-v1/ERRATA.md) also scopes a frozen preservation-field name to represented sources. These follow-up checks and clarifications do not change the original cases, protocol, comparison code or results.
...
Anthropic Claude Science, with its interface set to Opus 5 and maximum reasoning effort, also reviewed the public packet at commit `46a90676bd7279bf513ce62037a96b130abf726d`. Its saved review and the [checked disposition](../reviews/evidence-fit-2026-09-08.md) distinguish verified bytes and executed checks from interpretation and review limitations. This additional AI audit does not supply the pending human approvals or establish the validity of scientific extractions.
