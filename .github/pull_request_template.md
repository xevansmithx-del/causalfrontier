## Change

Describe the smallest user-visible or trust-boundary change.

## Evidence

- [ ] Offline tests pass on supported Python versions.
- [ ] Normal and `python -O` probe output is identical.
- [ ] Ruff, formatting, pre-commit, privacy, package, and exact-source checks pass.
- [ ] New branches, gates, parsers, and ledger mutations have adversarial coverage.
- [ ] The prospective benchmark remains `N=0`, or a preregistered benchmark change is explicitly reviewed.
- [ ] No causal, clinical, laboratory, material, or human-decision authority is implied.
- [ ] No private, restricted, patient-level, or unlicensed data is included.
