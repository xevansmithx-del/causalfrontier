...
Read the [protocol](PROTOCOL.md), [results and limits](RESULTS.md), and
...
uv run --frozen --no-sync python -O evaluation/evidence-fit-v1/run.py \
  --verify-existing evaluation/evidence-fit-v1/results
```

Replay checks current bound source identities, regenerates all artifacts in a
temporary directory and compares their complete file set and bytes. The stored
results are not modified. Replay works without Git metadata, including from the
source distribution. This component was added after packaged release v0.1.0a2;
use a development-source checkout containing this directory.
...
For a separate new initial-run output, use an absolute destination that does not
exist and the full prerequisite commit shown above:
